...
// Create a 3D complex array from multiplication of a 2D array by 1D array
//attempt to perform this operation via nested for loops

//wont work becuase it is not a parallelizable function!
__global__ void Create3DKernelv2(cufftComplex* KernelOut, cufftComplex* KernelIn, float* zscale, int size, int imgsize)
{
	const int numThreads = blockDim.x * gridDim.x;
	const int threadID = blockIdx.x * blockDim.x + threadIdx.x;
	...
	//its imperative that operations are performed on the individual elements,can not appyling the scaling simultaneously .x/.y
	for (int i = threadID; i < size; i += numThreads)
		//resets every numThread execution! Probably has to do with synchronisation!
	{
		j = (k < imgsize) ? j : (j + 1);
		k = (k < imgsize) ? k : 0 ;
		//float scale = zscale[j]; //temporary debug
		KernelOut[i].x = j;//*KernelIn[k].x;
		KernelOut[i].y = k;//*KernelIn[k].y;
		k++;
	}

...

{
  ...
  for (j = dim-1; j >= 0; j--)    // reverse order gives better results.
  {
    ...
    min = assigncost[0+j*rowSize]; 
    imin = 0;
    for (i = 1; i < dim; i++)  
      if (assigncost[i+j*rowSize] < min) 
      { 
        min = assigncost[i+j*rowSize]; 
        imin = i;
      }
    v[j] = min; 
  ...
  for (i = 0; i < dim; i++) 
    if (matches[i] == 0)     // fill list of unassigned 'free' rows.
      ...
      if (matches[i] == 1)   // transfer reduction from rows that are assigned once.
      {
        ...
        for (j = 0; j < dim; j++)  
          if (j != j1)
            if (assigncost[i+j*rowSize] - v[j] < min) 
              min = assigncost[i+j*rowSize] - v[j];
        v[j1] = v[j1] - min;
      }
  ...
  do
  {
    ...
    while (k < prvnumfree)
    {
      ...
      umin = assigncost[i+0*rowSize] - v[0]; 
      j1 = 0; 
      usubmin = BIG;
      for (j = 1; j < dim; j++) 
      {
        h = assigncost[i+j*rowSize] - v[j];
        if (h < usubmin)
          if (h >= umin) 
          { 
            usubmin = h; 
  ...
  for (f = 0; f < numfree; f++) 
  {
    ...
    for (j = 0; j < dim; j++)  
    { 
      d[j] = assigncost[freerow+j*rowSize] - v[j]; 
      pred[j] = freerow;
    ...
    do
    {
      ...
      if (!unassignedfound) 
      {
        ...
        low++; 
        i = colsol[j1]; 
        h = assigncost[i+j1*rowSize] - v[j1] - min;

        for (k = up; k < dim; k++) 
        {
          j = collist[k]; 
          v2 = assigncost[i+j*rowSize] - v[j] - h;
          if (v2 < d[j])
          {
            pred[j] = i;
  ...
  {
    j = rowsol[i];
    u[i] = assigncost[i+j*rowSize] - v[j];
    lapcost = lapcost + assigncost[i+j*rowSize]; 
  }
